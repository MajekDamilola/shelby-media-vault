...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      g1.addColorStop(0, "rgba(255,160,20,0.10)");
      g1.addColorStop(1, "transparent");
      ctx.fillStyle = g1;
      ctx.fillRect(0, 0, W, H);

      // Radial amber glow bottom-right
      const g2 = ctx.createRadialGradient(W * 0.9, H * 0.85, 0, W * 0.9, H * 0.85, W * 0.45);
      g2.addColorStop(0, "rgba(255,120,0,0.07)");
      ...
      // Hex grid
      const R = 42;
      const cols = Math.ceil(W / (R * 1.732)) + 3;
      const rows = Math.ceil(H / (R * 1.5)) + 3;
      ...
      for (let row = -1; row < rows; row++) {
        for (let col = -1; col < cols; col++) {
          ...
          // Filled accent hexes near glow center
          if (wave > 0.88 && proximity > 0.3) {
            hex(x, y, R - 3, `rgba(255,150,10,${alpha * 0.5})`, "", 0);
          }

          // Stroke all hexes
          hex(x, y, R - 2, "", `rgba(255,140,0,${alpha})`, 0.7);

          // Bright center dot on some hexes
          if (wave > 0.95 && proximity > 0.4) {
            ctx.beginPath();
            ctx.arc(x, y, 1.5, 0, Math.PI * 2);
            ctx.fillStyle = `rgba(255,180,40,${proximity * 0.8})`;
            ctx.fill();
          }
        }
      }
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", position: "relative", background: "#080806", overflow: "hidden" }}>
      ...
      {/* CONTENT */}
      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", minHeight: "100vh", fontFamily: "'Space Grotesk', sans-serif" }}>
        ...
        <nav style={{
          ...
          background: "rgba(8,8,6,0.7)", backdropFilter: "blur(16px)",
        }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <a href="https://docs.shelby.xyz/apis/faucet/shelbyusd" target="_blank" rel="noopener noreferrer"
              style={{
                padding: "8px 18px", fontSize: 13, fontWeight: 700, color: "#080806",
                background: "linear-gradient(135deg, #ffaa00, #ff7a00)",
                borderRadius: 8, textDecoration: "none", letterSpacing: "0.01em",
                boxShadow: "0 2px 16px rgba(255,140,0,0.25)", transition: "all 0.18s",
              }}
              onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 4px 28px rgba(255,140,0,0.45)"; e.currentTarget.style.transform = "translateY(-1px)"; }}
              onMouseLeave={e => { e.currentTarget.style.boxShadow = "0 2px 16px rgba(255,140,0,0.25)"; e.currentTarget.style.transform = ""; }}>
              Get Testnet Funds
            </a>
          </div>
        </nav>
          ...
          <div style={{ width: "100%", maxWidth: 1120, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 420px", gap: 96, alignItems: "center" }}>
            ...
            <div>
              ...
              <div className="anim-fade-up" style={{
                ...
                border: "1px solid rgba(255,140,0,0.25)", borderRadius: 100,
                padding: "5px 14px 5px 10px", background: "rgba(255,140,0,0.06)",
              }}>
                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 18, height: 18 }}>
                  <HexLogo size={18} />
                </span>
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#ffaa00" }}>
              ...
              <h1 className="anim-fade-up delay-2" style={{
                fontSize: "clamp(44px, 6vw, 76px)", fontWeight: 800,
                lineHeight: 1.0, letterSpacing: "-0.035em", marginBottom: 28,
              ...
              <p className="anim-fade-up delay-3" style={{
                fontSize: 17, lineHeight: 1.75, color: "rgba(255,255,255,0.5)",
              ...
              <div className="anim-fade-up delay-4" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, maxWidth: 480, marginBottom: 48 }}>
                {[
                  ["⬡", "Decentralized Storage", "Files live on Shelby's network"],
                  ["◎", "Wallet-Based Access", "Your keys, your vault"],
                  ["⟳", "Fast Retrieval", "Hot storage, instant reads"],
                  ["⎘", "Shareable Links", "Public URLs for any file"],
                ].map(([icon, title, sub]) => (
                  <div key={title} style={{
            ...
            <div className="anim-scale-in delay-2" style={{ position: "relative" }}>
              ...
              <div style={{
                ...
                borderRadius: 20, padding: 36, overflow: "hidden",
              }}>
                {/* Top amber line */}
                <div style={{
                ...
                <svg style={{ position: "absolute", top: -12, right: -12, opacity: 0.15 }} width={80} height={80} viewBox="0 0 80 80">
                  {[[40,40,28],[40,40,18],[40,40,10]].map(([cx,cy,r],i) => (
                    <polygon key={i} points={Array.from({length:6},(_,j)=>{
                      const a=(Math.PI/3)*j-Math.PI/6;
                      return `${cx+r*Math.cos(a)},${cy+r*Math.sin(a)}`;
                    }).join(" ")} fill="none" stroke="#ffaa00" strokeWidth="0.8" />
                  ))}
                </svg>

                <div style={{ marginBottom: 24 }}>
                  <h3 style={{ fontSize: 19, fontWeight: 800, color: "#fff", marginBottom: 6, letterSpacing: "-0.02em" }}>Connect Wallet</h3>
                  <p style={{ fontSize: 13, color: "rgba(255,255,255,0.38)", lineHeight: 1.6 }}>
                    Enter your Aptos devnet credentials.{" "}
                    <span style={{ color: "#ffaa00" }}>Never use mainnet keys.</span>
                  </p>
                </div>

                <form onSubmit={handleConnect} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div>
                    <label style={{ display: "block", fontSize: 11, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)", marginBottom: 8 }}>Account Address</label>
                    <input
                      ...
                      onFocus={e => { e.target.style.borderColor = "#ffaa00"; e.target.style.boxShadow = "0 0 0 3px rgba(255,170,0,0.1)"; }}
                      onBlur={e => { e.target.style.borderColor = "rgba(255,140,0,0.15)"; e.target.style.boxShadow = ""; }}
                    />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: 11, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)", marginBottom: 8 }}>Private Key</label>
                    <input
                  ...
                  {error && (
                    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "rgba(255,80,80,0.08)", border: "1px solid rgba(255,80,80,0.2)", borderRadius: 8, fontSize: 13, color: "#ff6060" }}>
                      ✕ {error}
                    </div>
                  )}

                  <button type="submit" style={{
                    marginTop: 4, padding: "13px 24px", width: "100%",
                    background: "linear-gradient(135deg, #ffcc00, #ff8800)",
                    border: "none", borderRadius: 10, cursor: "pointer",
                    fontSize: 15, fontWeight: 800, color: "#080806",
                    letterSpacing: "-0.01em",
                    boxShadow: "0 4px 28px rgba(255,140,0,0.35)",
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[["ShelbyUSD Faucet", "https://docs.shelby.xyz/apis/faucet/shelbyusd"], ["CLI Setup", "https://docs.shelby.xyz/tools/cli"]].map(([label, href]) => (
                    <a key={label} href={href} target="_blank" rel="noopener noreferrer"
                      ...
                      onMouseEnter={e => { e.currentTarget.style.color = "#ffaa00"; e.currentTarget.style.borderColor = "rgba(255,140,0,0.3)"; e.currentTarget.style.background = "rgba(255,140,0,0.06)"; }}
                      onMouseLeave={e => { e.currentTarget.style.color = "rgba(255,255,255,0.3)"; e.currentTarget.style.borderColor = "rgba(255,140,0,0.08)"; e.currentTarget.style.background = "rgba(255,255,255,0.03)"; }}>
                      {label} →
                    </a>
                  ))}
                </div>
        ...
        <footer style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <HexLogo size={20} />
            <span style={{ fontSize: 12, color: "rgba(255,255,255,0.2)" }}>Shelby Media Vault · Devnet</span>
          </div>
          <div style={{ display: "flex", gap: 24 }}>
            {[["shelby.xyz", "https://shelby.xyz"], ["Docs", "https://docs.shelby.xyz"], ["GitHub", "https://github.com/shelby/shelby-quickstart"]].map(([l, h]) => (
...
function HexLogo({ size = 36 }: { size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ flexShrink: 0 }}>
      ...
      <polygon points={pts} fill="url(#hg)" />
      <polygon points={innerPts} fill="rgba(8,8,6,0.5)" />
      <circle cx={cx} cy={cy} r={r * 0.22} fill="url(#hg)" />
    </svg>
  );
}
