    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 32, position: "relative", zIndex: 1 }}>
      ...
      <div className="anim-fade-up" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16, paddingTop: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn btn-ghost btn-sm" onClick={handleRefetch}>↻ Refresh</button>
          <button className="btn btn-primary" onClick={() => { setShowUpload(true); resetUpload(); }}
            style={{ background: "linear-gradient(135deg, #ffcc00, #ff8800)", color: "#080806", fontWeight: 800 }}>
            + Upload
          </button>
        </div>
      </div>
      ...
      <div className="anim-fade-up delay-1" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px,1fr))", gap: 10 }}>
        {[
          ...
          <div key={stat.label} style={{
            background: "var(--surface)", border: "1px solid rgba(255,140,0,0.08)",
            borderRadius: 14, padding: "16px 20px",
          }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: i === 0 ? "#ffaa00" : "var(--text-1)", letterSpacing: "-0.02em", lineHeight: 1 }}>{stat.value}</div>
            <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 6, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 700 }}>{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="anim-fade-up delay-2" style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 4, background: "var(--surface-2)", borderRadius: 10, padding: 4, border: "1px solid rgba(255,140,0,0.06)" }}>
          {FILTER_TABS.map(tab => (
            <button key={tab.id} onClick={() => setFilter(tab.id)} style={{
              ...
              color: filter === tab.id ? "#ffaa00" : "var(--text-3)",
      ...
      {showUpload && (
        <UploadZone uploadProgress={uploadProgress} onUpload={handleUpload} onClose={() => setShowUpload(false)} />
      )}
      {selected && (
        <FilePreviewModal file={selected} onClose={() => setSelected(null)}
          onCopyLink={() => handleCopyLink(selected)} />
      )}
    </div>
  );
}
